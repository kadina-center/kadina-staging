...
function assertCredentials(): { accessToken: string; pageId: string } {
  const accessToken = env.MESSENGER_ACCESS_TOKEN;
  ...
  if (
    ...
    throw new Error(
      "Messenger credentials are not configured. Set MESSENGER_ACCESS_TOKEN and MESSENGER_PAGE_ID in .env"
    );
  }
...
export async function sendMessengerText(
  ...
  try {
    ...
    if (!platformMessageId) {
      throw new Error("Messenger API response did not include a message id");
    }
    return { platformMessageId };
  } catch (error) {
    throw new Error(metaErrorMessage(error, "Failed to send Messenger message"));
  }
}
...
export function parseMessengerWebhook(payload: unknown): ParseWebhookResult {
...
export const messengerAdapter: ChannelAdapter = {
  ...
  async sendMessage(to, content) {
    return sendMessengerText(to, content);
  },
  parseIncomingWebhook: parseMessengerWebhook,
};
